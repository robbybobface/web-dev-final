...
const Search = () => {
  ...
  const getRecommended = async () => {
    ...
    const localFavTracks = stateUser.likedSongs
      .map((song) => song.songId)
      .slice(0, 2)
      .join(",");
    const localFavArtists = stateUser.likedArtists
  ...
  useEffect(() => {
    ...
    if (stateLoggedIn && stateUser) {
      if (
        stateUser.likedSongs.length > 0 &&
        stateUser.likedArtists.length > 0 &&
  ...
  return (
    <>
      <Helmet>
        <style>
          {"body {   background-image: url('https://images.unsplash.com/photo-1614854262318-831574f15f1f?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80') !important;\n" +
            "  background-repeat: no-repeat !important;\n" +
            "  background-size: cover !important;\n" +
            "  background-color: rgba(61, 162, 195, 0.1) !important; }"}
        </style>
      </Helmet>
            ...
            <div className="card-body">
              ...
              <div className="row justify-content-center">
                ...
                <div className="col-md-2">
                  <input
                    className="btn-hover-search color-8"
                    type="submit"
                    value="Search"
              ...
              {!stateLoggedIn ? (
                ""
              ) : stateUser.likedArtists.length > 1 &&
                stateUser.likedSongs.length > 1 &&
                stateUser.likedGenres.length > 0 ? (
                !getRecommendations ? (
                  <div className="row d-flex mt-4 mb-0 align-items-center justify-content-center">
                    <div className="d-flex align-items-center justify-content-center">
                      <span className="recommended-text">
                        Want some recommended tracks?
                      </span>
                      <div className="col-md-2">
                        <input
                          className="btn-hover-rec color-8"
                          type="submit"
                  ...
                  <div className="row d-flex mt-4 mb-0 align-items-center justify-content-center">
                    <div className="d-flex align-items-center justify-content-center">
                      <span className="recommended-text">
                        Back to Top 50 US Songs
                      </span>
                      <div className="col-md-2">
                        <input
                          className="btn-hover-back color-8"
                          type="submit"
                          value="Go Back"
                          onClick={recommendedHandler}
                        />
        ...
        {loading ? (
          ...
          <>
            <h1 className="search-category text-center py-3">
              <span className="category-underline">Tracks</span>
            </h1>
          ...
          <>
            <h1 className="search-category text-center py-3">
              <span className="category-underline">Artists</span>
            </h1>
            <MDBRow className="row-cols-2 row-cols-md-2 row-cols-lg-4 g-4 flex-row">
          ...
          <>
            <h1 className="search-category text-center py-3">
              <span className="category-underline">Albums</span>
            </h1>
            <MDBRow className="row-cols-2 row-cols-md-2 row-cols-lg-4 g-4 flex-row">
              ...
              {topSongs.map((track, index) => (
                <div key={index}>
                  ...
                  >
                    <div
                      onClick={() => {
                        trackNavigate(track.track.id);
                        window.scrollTo(0, 0);
                      }}
                      key={index + 1}
                    >
                      <TrackListItem key={track.track.id} track={track.track} />
                    </div>
                  </MDBCol>
                </div>
              ))}
